...
log_info "Checking installation directory: $CLAUDE_DIR"

if [ ! -d "$CLAUDE_DIR" ]; then
    log_warning "~/.claude directory does not exist. Creating it."
    mkdir -p "$CLAUDE_DIR"
    log_success "Directory created successfully"
else
    log_success "Directory verified"
fi
...
log_info "Copying statusline.sh..."
...
log_success "statusline.sh installed successfully"
...
if [ -f "$SETTINGS_PATH" ]; then
    ...
    log_success "Existing settings.json updated successfully"
else
...
echo ""
log_info "Statusline features:"
echo "  🤖 Model name display"
echo "  📁 Current directory"
echo "  🌿 Git branch and status"
echo "  ⛽ Token usage (fuel gauge)"
echo ""
